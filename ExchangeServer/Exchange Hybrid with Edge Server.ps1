<#-------------------------------------------Exchange Hybrid Configuration with Edge Server-------------------------------------------#>
...
<#------------------------------------------- Phase 3: Edge Server - After HCW-------------------------------------------#>

    # 0. HCW8056 - Receive connector on the following Edge Transport servers need to be manually configured: JOSEEDGE01
        Set-ReceiveConnector -Identity "JOSEEDGE01\Default internal receive connector JOSEEDGE01" -TlsDomainCapabilities mail.protection.outlook.com:AcceptOorgProtocol -Fqdn 'jose.run.place'
    
    # 1. Certificate
        # Import Public Certificate of jose.run.place and assign SMTP Services. Lastly, execute iisreset.